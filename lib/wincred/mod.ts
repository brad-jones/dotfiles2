...
export async function installCredentialManager(version: string, d: Digest) {
  if (OS !== "windows") {
    console.log("wincred | skipping CredentialManager install, not windows");
    return;
  }
  ...
  if (await fs.exists(moduleDir)) {
    console.log("wincred | CredentialManager is already installed");
    return;
  }
  ...
  console.log("wincred | installing CredentialManager");
